...
static func from_dict(data: Dictionary) -> Problem:
	...
	var target_data: Dictionary = data.get("target_vector", {"x": 0.0, "y": 0.0}) as Dictionary
	problem.target_vector = Vector2(float(target_data.get("x", 0.0)), float(target_data.get("y", 0.0)))
